...
def etl_load_stage(table: str, columns: list, stage: str, path: str):
...
def etl_load_dim_customer_flow():
    etl_load_stage('customer', ['name', 'birth_date', 'town', 'email'], 'mdw_staging', 'customers')
    etl_load_prod('customer')

@flow
def etl_load_dim_product_flow():
    etl_load_stage('product', ['ean', 'category', 'net_price'], 'mdw_staging', 'products')
    etl_load_prod('product')

@flow
def etl_load_dim_shop_flow():
    etl_load_stage('shops', ['id', 'city'], 'mdw_staging', 'shops')
    etl_load_prod('shops')
